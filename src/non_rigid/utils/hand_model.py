...
class HandModel:
    def __init__(
        ...
        self.vertices = {}
        removed_links = json.load(open(os.path.join(data_dir, 'data_utils/removed_links.json')))[robot_name]
...
def create_hand_model(
    ...
    json_path = os.path.join(data_dir, 'data/data_urdf/robot/urdf_assets_meta.json')
    urdf_assets_meta = json.load(open(json_path))
    urdf_path = os.path.join(data_dir, urdf_assets_meta['urdf_path'][robot_name])
    meshes_path = os.path.join(data_dir, urdf_assets_meta['meshes_path'][robot_name])
    links_pc_path = os.path.join(data_dir, f'data/PointCloud/robot/{robot_name}.pt')
    hand_model = HandModel(robot_name, urdf_path, meshes_path, links_pc_path, device, num_points, data_dir)
    return hand_model
